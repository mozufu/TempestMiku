...
subtree cancel-on-sibling-failure, DAG/protocol enforcement, fuller provenance); product approval surfaces for
...
| 6+ | **P3+ — live actor mailbox + supervision** | 4–8d | Foundation landed: per-actor bounded inbox + `Agent::run` inbox draining, `agents.send/broadcast/wait/inbox/list/cancel`, child approval routing through the live broker, parent-session child artifacts, parent-driven cancel tokens with replayable `actor_cancelled`, and tm-e2e/Flutter actor smoke coverage. Remaining: `pipeline`; active restart strategies (`one_for_one`, `one_for_all`, `rest_for_one`); wall-clock budget enforcement; subtree cancel-on-sibling-failure; DAG acyclicity check; plain-prose enforcement; fuller provenance. | Live siblings coordinate via `send`/`wait`; child cancel yields replayable `Cancelled` event; child approval requests resolve through the live broker; Flutter attach/approve/reconnect smoke tests pass. |
...
1. **Continue P3+** — the live inbox foundation, child approval routing, and actor smoke coverage are
   in place; next tackle pipeline, restart strategies, cancel tokens, and protocol/provenance
   hardening.
