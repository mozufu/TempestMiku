...
| **P3+ — live actor mailbox** | Live MPSC resident delivery (`agents.send/wait/inbox/list/broadcast/cancel/pipeline`); active supervision (restart/subtree cancel); child approval routing through `ApprovalBroker`; DAG acyclicity + protocol invariants; Flutter smoke coverage | 5 | M2 | live siblings coordinate via `send`/`wait`; child cancel yields replayable `Cancelled` event; child approval requests reach the user; Flutter attach/approve/reconnect pass |
...
## Execution plan from current workspace (2026-07-05)
...
draining, live `agents.send/wait/inbox/list/cancel`, delivered-message lifecycle events, actor-id
threading through `InvocationCtx`, child approvals routed through the live `HttpApprovalPolicy` /
`ApprovalBroker`, parent-session child artifacts, parent-driven actor cancel tokens with replayable
`actor_cancelled` events, and tm-e2e/Flutter actor smoke coverage.
...
scheduler/dreaming; the remaining P3+ actor surface (pipeline, active restart strategies,
subtree cancel-on-sibling-failure, fuller provenance); product approval surfaces for
...
| 6+ | **P3+ — live actor mailbox + supervision** | 4–8d | Foundation landed: per-actor bounded inbox + `Agent::run` inbox draining, `agents.send/broadcast/wait/inbox/list/cancel`, child approval routing through the live broker, parent-session child artifacts, parent-driven cancel tokens with replayable `actor_cancelled`, plain-prose enforcement, current live-wait DAG acyclicity checks, and tm-e2e/Flutter actor smoke coverage. Remaining: `pipeline`; active restart strategies (`one_for_one`, `one_for_all`, `rest_for_one`); wall-clock budget enforcement; subtree cancel-on-sibling-failure; fuller provenance. | Live siblings coordinate via `send`/`wait`; child cancel yields replayable `Cancelled` event; child approval requests resolve through the live broker; Flutter attach/approve/reconnect smoke tests pass. |
...
1. **Continue P3+** — the live inbox foundation, child approval routing, and actor smoke coverage are
   in place; next tackle pipeline, restart strategies, wall-clock budgets, subtree cancellation,
   and fuller provenance.
...
| `agents.*` | **P3 ✓ / P3+** | P3 MVP shipped: `agents.run/spawn/parallel/msg`, actor lifecycle, mailbox/roster, `agent://`+`history://` resources, SSE lifecycle events, Handoff mode. P3+ foundation shipped: bounded live inbox delivery plus `send`, `broadcast`, `wait`, `inbox`, `list`, and `cancel`, child approval routing, plain-prose enforcement, current live-wait DAG acyclicity checks, and actor smoke coverage. Remaining P3+ owns `pipeline`, active restart strategies, subtree cancellation policy, and fuller provenance. |
