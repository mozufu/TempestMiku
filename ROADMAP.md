...
draining, live `agents.send/wait/inbox/list`, delivered-message lifecycle events, actor-id
threading through `InvocationCtx`, child approvals routed through the live `HttpApprovalPolicy` /
`ApprovalBroker`, parent-session child artifacts, and tm-e2e/Flutter actor smoke coverage.
...
scheduler/dreaming; the remaining P3+ actor surface (broadcast/pipeline, restart/cancel,
DAG/protocol enforcement, fuller provenance); product approval surfaces for
...
| 6+ | **P3+ — live actor mailbox + supervision** | 4–8d | Foundation landed: per-actor bounded inbox + `Agent::run` inbox draining, `agents.send/wait/inbox/list`, child approval routing through the live broker, parent-session child artifacts, and tm-e2e/Flutter actor smoke coverage. Remaining: `broadcast/pipeline`; active restart strategies (`one_for_one`, `one_for_all`, `rest_for_one`); cancel tokens; wall-clock budget enforcement; subtree cancel-on-sibling-failure; DAG acyclicity check; plain-prose enforcement; fuller provenance. | Live siblings coordinate via `send`/`wait`; child cancel yields replayable `Cancelled` event; child approval requests resolve through the live broker; Flutter attach/approve/reconnect smoke tests pass. |
...
1. **Continue P3+** — the live inbox foundation, child approval routing, and actor smoke coverage are
   in place; next tackle broadcast/pipeline, restart strategies, cancel tokens, and protocol/provenance
   hardening.
...
| `agents.*` | **P3 ✓ / P3+** | P3 MVP shipped: `agents.run/spawn/parallel/msg`, actor lifecycle, mailbox/roster, `agent://`+`history://` resources, SSE lifecycle events, Handoff mode. First P3+ foundation shipped: bounded live inbox delivery plus `send`, `wait`, `inbox`, and `list`, child approval routing, and actor smoke coverage. Remaining P3+ owns `pipeline`, `broadcast`, restart/cancel, DAG/protocol enforcement, and fuller provenance. |
