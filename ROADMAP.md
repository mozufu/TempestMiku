...
| **P1 — project manager** | Chief-of-staff layer over the coding loop: mode router, project plans/open loops/decisions, session event log, SSE/WebUI project surface | 1/2/4 | M1–M2 | router fires; user can lock; badge + voice cap work; project status survives reconnect; Miku can turn a coding session into next actions without losing provenance |
...
| **P6 — Android** | thin Android client (§27.4) | all | — | Android + WebUI both connect to the same server |
...
| 4 | **P1 — project manager** | 4–7d | Add `tm-server` (`axum`) custom session API + SSE, Postgres-backed session events/messages/project notes, mode router lock/override, `ModeChanged` events, and project/open-loop views. | Reconnect replays from Postgres event id; router fires; user can lock; project status and decisions survive across sessions; Miku turns completed coding work into next actions with provenance. |
...
| 9 | **P6 — Android thin client** | 4–7d | Build Android chat/badge/browser client against existing server API; no on-device sandbox; shared auth/pairing. | Android and WebUI attach to the same server/session stream; reconnect/resume works on mobile. |
...
- Android before P1/P2 server APIs stabilize.
