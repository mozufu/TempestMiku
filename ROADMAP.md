...
1. **Deferred — production P6 push provider** — Firebase/FCM setup and integration are paused while
   available Firebase capacity is exhausted. Keep push disabled and do not create project config,
   client SDK wiring, or server credentials. Revisit only when capacity is available again or the user
