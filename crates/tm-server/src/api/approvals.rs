    ...
    S: Store,
{
    ...
    let proposal_payload = match lease.effect.effect_type.as_str() {
        ...
        "persona_addendum_rollback" => {
            apply_persona_addendum_rollback_effect(
                approval,
                &lease.effect,
                self_evolution_tier,
                persona,
...
async fn apply_persona_addendum_rollback_effect(
    approval: &ApprovalRequestRecord,
    effect: &ApprovalEffectRecord,
    self_evolution_tier: SelfEvolutionTier,
    persona: &tm_modes::ModesConfig,
    ...
    let rollback = effect
        ...
        .cloned()
        .ok_or_else(|| {
            ServerError::Store("persona addendum rollback effect missing rollback".to_string())
        })?;
    let persona_id = rollback
        .get("personaId")
        .and_then(Value::as_str)
        .ok_or_else(|| {
            ServerError::Store("persona addendum rollback effect missing personaId".to_string())
        })?;
    let expected_active_digest = rollback
        .get("expectedActiveDigest")
        .and_then(Value::as_str)
        .ok_or_else(|| {
            ServerError::Store(
                "persona addendum rollback effect missing expectedActiveDigest".to_string(),
            )
        })?;
    let target_digest = rollback.get("targetDigest").and_then(Value::as_str);
    let evolution_target_id = rollback
        ...
        .ok_or_else(|| {
            ServerError::Store(
                "persona addendum rollback effect missing evolutionTargetId".to_string(),
            )
        })?;
    let status = approval_terminal_status(&approval.status)?;
    let activation = if approval.status == "approved" {
        ...
        crate::evolution::verify_approved_review_effect(
            self_evolution_tier,
            &effect.payload_json,
            tm_host::EvolutionTargetClass::PersonaProposal,
            evolution_target_id,
            &digest,
        )?;
        let managed = persona
        ...
        {
            return Err(ServerError::NotFound(format!(
                "managed persona addendum {persona_id} version {target_digest}"
            )));
        }
        Some(
            persona
                .rollback_managed_persona_addendum(
                    persona_id,
    ...
    Ok(
        crate::api::sessions::evolution_review::persona_addendum_rollback_payload(
            &rollback,
            status,
            activation.as_ref(),
        ),
