...
- **Resource resolver registry** — one scheme-dispatched `read(uri)` over registered handlers
  ...
  `agent://` / `history://` / `cron://`, configured `drive://`, managed `skill://` versions,
  and configured `mcp://` resources; every optional handler still requires its scheme grant);
  §9.2.
- **MCP import runtime** — `tm-mcp` discovers only operator-selected tools, prompts, and resources
  through the egress/opaque-secret boundary, then exposes lazy `mcp.<alias>.*` code capabilities
...
- **Secret broker** — `tm-egress` resolves exact session/actor/destination-scoped opaque handles
