...
- `agents.run/spawn/parallel/msg/send/broadcast/wait/inbox/list(...)` — capability-gated sub-agent
  orchestration, defined only in sessions holding an `agents.*` grant. Ungranted sessions keep
  `agents` as `undefined`; the remaining P3-plus/full surface still owns `pipeline` and active
  supervision.
 ...
 * delivery through send, broadcast, wait, inbox, and list. Child approval routing
 * through the live HTTP broker is now in place; pipeline, cancel, restart, and
 * stricter protocol enforcement remain later P3-plus work.
 */
