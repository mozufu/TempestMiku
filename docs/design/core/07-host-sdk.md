...
- `agents.run/spawn/parallel/pipeline/msg/send/broadcast/wait/inbox/list/cancel(...)` —
  ...
  Ungranted sessions keep `agents` as `undefined`; remaining P3-plus work owns active supervision,
  wall-clock budgets, subtree cancellation, and fuller provenance.
 ...
 * P3 shipped run, spawn, parallel, and msg; P3-plus adds live per-actor inbox
 * delivery through send, broadcast, wait, inbox, list, cancel, and pipeline.
 * Child approval routing through the live HTTP broker is now in place; restart
 * supervision, wall-clock budgets, subtree cancellation, and fuller provenance
