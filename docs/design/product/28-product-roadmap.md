...
| 3 | **P0 — character server slice** | 5–8d | Add `tm-server` session API + SSE; add minimal `tm-persona` voice overlay; load SOUL / skills as persona assets; add minimal `tm-memory` profile + recall store sufficient for P0; WebUI first chat stream. | Browser/user smoke: token-by-token Miku response over SSE, mode badge default, profile/recall context injected, reconnect replays from event id. |
| 4 | **P1 — modes + serious engineer** | 5–8d | Implement 5-mode router, lock/override, `ModeChanged` events; add linked-folder `FsPolicy`; implement `fs.*`, `code.search/find/edit` first, `proc.run(cmd,args)` allowlist + approval. | Real linked repo edit+test succeeds through SDK; destructive/external commands are approval-gated; serious mode voice cap is 關. |
